 ...
 * 只能抓 5 天內的天氣，所以如果超過 5 天就不會有天氣資料
 */
@Service
...
public class WeatherServiceImpl implements WeatherService {
    ...
    /**
     * Fetches weather data from OpenWeatherMap API, summarizes daily information,
     * and persists the results to the TripWeather table.
     */
    @Override
    @Transactional
    public void fetchAndStoreWeather(TripPreference preference) {
        validatePreference(preference);  // Ensure input is valid
    ...
    private URI buildForecastUri(TripPreference preference) {
        String location = Stream.of(preference.getCity(), preference.getCountry())
